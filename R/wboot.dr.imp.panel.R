...
wboot.dr.imp.panel <- function(nn, n, deltaY, D, int.cov, i.weights, nthreads){
  #-----------------------------------------------------------------------------
  v <- stats::rexp(n)
  #v <- v / mean(v)
  ...
  ps.b <- pscore.cal(D, int.cov, b.weights, n, nthreads = nthreads)$pscore
  ps.b <- pmin(ps.b, 1 - 1e-16)
  #Compute the Outcome regression for the control group
  out.reg.b <- wols.br.panel(deltaY, D, int.cov, ps.b, i.weights = b.weights, nthreads = nthreads)$out.reg
